...
const POINT_ALIASES: Record<string, string> = {
  ...
  // 充电点别名由地图数据动态注入（通过 updatePointAliases）
...
function tryParseCharge(text: string): ParsedTask | null {
  if (CHARGE_KEYWORDS.some(k => text.includes(k))) {
    return {
      ...
      confidence: 0.9,
      rawText: text,
      description: "前往充电站充电（充电点由地图配置决定）",
      subtaskPreview: [
        { step: 1, action: "移动到充电点", target: "地图充电点", offset1: 1 },
        { step: 2, action: "对接充电", target: "地图充电点", offset1: 4 },
      ],
    };
  }
  return null;
