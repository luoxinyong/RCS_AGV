...
void DistortionAdjust::AdjustRsCloud(CloudData::CLOUD_PTR& input_cloud_ptr, CloudData::CLOUD_PTR& output_cloud_ptr) {

    // 定义起始和结束的方位角
    // 需要注意第一个点与最后一个点可能存在NAN值，如果已过滤，也可以用下面代码
    float start_orientation = -atan2(input_cloud_ptr->points.front().y, input_cloud_ptr->points.front().x);
    float end_orientation = -atan2(input_cloud_ptr->points.back().y, input_cloud_ptr->points.back().x);

    float orientation_diff = end_orientation - start_orientation;
    if (orientation_diff < 0) {
        orientation_diff += 2 * M_PI;  // 确保方位角差是正值
    }
    ROS_ERROR(" start_end_orientation: [%f, %f], orientation_diff: %f", start_orientation, end_orientation, orientation_diff);

    start_orientation = 0.0;
    orientation_diff = 2 * M_PI;
    output_cloud_ptr->points.resize(input_cloud_ptr->points.size());
    ...
    for (size_t point_index = 0; point_index < input_cloud_ptr->points.size(); ++point_index) {
        ...
        // ROS_ERROR(" index:  %d,  half_passed: %d,  ori: %f,    point: [%f,     %f,     %f]", point_index, half_passed, ori, point(0), point(1), point(2));

        float real_time = (ori - start_orientation) / orientation_diff * scan_period_;
        current_matrix = UpdateMatrix(real_time);
        rotated_point = current_matrix * point;
        adjusted_point = rotated_point + velocity_ * real_time;
        output_cloud_ptr->points[point_index].x = adjusted_point(0);
        output_cloud_ptr->points[point_index].y = adjusted_point(1);
        output_cloud_ptr->points[point_index].z = adjusted_point(2);
    }
