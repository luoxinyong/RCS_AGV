...
void vel_callback(const geometry_msgs::TwistWithCovarianceStampedConstPtr& msg) {
  geometry_msgs::TwistStamped vel_data;
  double time = ros::Time::now().toSec();  // 把时间戳转化成浮点型格式
  vel_data.header.stamp = ros::Time().fromSec(time);  // 把浮点型变成时间戳
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber vel_sub = nh_.subscribe("/oxts_ins/vel", 10, vel_callback);
