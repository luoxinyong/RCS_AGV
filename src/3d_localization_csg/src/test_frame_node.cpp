...
int main(int argc, char *argv[]) {
  ...
  std::string config_file_path =
      ros::package::getPath("loc_ad") + "/config/front_end/front_end.yaml";
  YAML::Node config_node = YAML::LoadFile(config_file_path);
  std::string pointCloudTopic_ =
      config_node["pointCloudTopic"].as<std::string>();
  ROS_ERROR(" pointCloudTopic_ is %s", pointCloudTopic_.c_str());
  std::cout << " pointCloudTopic_ is " << pointCloudTopic_;
  ...
  while (ros::ok()) {
    ...
    if (!transform_received) {
      if (lidar_to_imu_ptr->LookupData(lidar_to_imu)) {
        ...
        // LOG(INFO) << "lidar to imu transform matrix is:" << std::endl <<
        // lidar_to_imu;
      }
    } else {
      while (cloud_data_buff.size() > 0 && imu_data_buff.size() > 0 &&
        ...
        if (d_time < -0.05) {
          ...
          pcl::transformPointCloud(*cloud_data.cloud_ptr, *cloud_data.cloud_ptr,
                                   odometry_matrix);
