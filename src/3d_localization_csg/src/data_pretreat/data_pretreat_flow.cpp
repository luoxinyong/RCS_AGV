...
namespace h_x {
DataPretreatFlow::DataPretreatFlow(ros::NodeHandle& nh, const std::string& cloud_topic) {
    std::string config_file_path =
        ros::package::getPath("loc_ad") + "/config/pretreat/pretreat.yaml";
    ...
    std::string gps_frame = config_node["gpsFrame"].as<std::string>();
    ...
    imu_to_gps_ptr_ = std::make_shared<TFListener>(nh, gps_frame, imu_frame);
...
bool DataPretreatFlow::ReadData() {
    ...
    if (!sensor_inited) {
        if (!valid_imu || !valid_velocity || !valid_gnss) {
            ROS_INFO(" valid_imu, valid_velocity, valid_gnss: [%d, %d, %d]", valid_imu, valid_velocity, valid_gnss);
...
bool DataPretreatFlow::InitCalibration() {
    ...
    if (!calibration_received) {
        if (lidar_to_imu_ptr_->LookupData(lidar_to_imu_) && imu_to_gps_ptr_->LookupData(imu_to_gps_)) {
...
bool DataPretreatFlow::LoadMapOriginInfo(double& latitude, double& longitude, double& altitude) {
    std::string config_filter_path = ros::package::getPath("loc_ad") + "/config/filtering/filtering.yaml";
    YAML::Node config_filter_node = YAML::LoadFile(config_filter_path);
    std::string path_map = config_filter_node["map_path"].as<std::string>();
    std::string path_map_info_file = path_map + "/map_info.yaml";
    YAML::Node config_map_info = YAML::LoadFile(path_map_info_file);
    latitude = config_map_info["latitude"].as<std::double_t>();
    longitude = config_map_info["longitude"].as<std::double_t>();
    altitude = config_map_info["altitude"].as<std::double_t>();
    ROS_INFO(" map_info -> latitude, longitude, altitude: [%f, %f, %f]", latitude, longitude, altitude);
    return true;
}
...
bool DataPretreatFlow::InitGNSS() {
    ...
    if (!gnss_inited) {
        ...
        LoadMapOriginInfo(latitude, longitude, altitude);
        GNSSData gnss_data = gnss_data_buff_.front();
        gnss_data.InitOriginPosition(latitude, longitude, altitude);
        gnss_inited = true;
    }
...
bool DataPretreatFlow::HasData() {
    ...
    if (velocity_data_buff_.size() == 0) {
        ROS_ERROR(" vel_data_buff size is 000000 ");
        return false;
    }
...
bool DataPretreatFlow::ValidData() {
    ...
    if (diff_velocity_time > 0.05) {
        ROS_ERROR(" vel_data_buff pop_front ");
...
bool DataPretreatFlow::TransformData() {
    ...
    gnss_pose_ *= imu_to_gps_;   // gps transform to imu 
    ...
    // if(!test) {
    //     distortion_adjust_ptr_->AdjustRsCloud(current_cloud_data_.cloud_ptr, current_cloud_data_.cloud_ptr);
    //     test = true;
    // }
