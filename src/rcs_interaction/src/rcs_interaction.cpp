...
{
    nh_.param<std::string>("/ip_rcs", params_rcs_.rcs_ip, std::string("10.177.156.195"));
    nh_.param<std::string>("/topic_task_rcs", topic_task_rcs_, "task_rcs");
    nh_.param<int>("/port",           params_rcs_.rcs_port, 1500);
    ...
    pub_door_stop_ = nh_.advertise<std_msgs::Int8>("coil_status", 10);
...
bool RcsInteraction::IsPortAvailable(const std::string& ip, int port) {
    int sock = socket(AF_INET, SOCK_STREAM, 0);
    if (sock < 0) {
        ROS_WARN("Socket test creation failed");
        return false;
    }
...
bool RcsInteraction::InitSocket()
{
    ...
    // if (!IsPortAvailable(params_rcs_.rcs_ip, params_rcs_.rcs_port)) {
    //     ROS_ERROR("Port %d is already in use or inaccessible", params_rcs_.rcs_port);
    //     return false;
    // }
    //创建modbus上下文并进行异常处理
    ...
    // 内存映射的配置,根据yaml文件或者默认参数
    m_header_length_ = modbus_get_header_length(m_ctx_);
    // std::cout<<"length = "<<m_header_length_<<std::endl;
    ...
    for (int i = 0; i < COIL_COUNT && i < params_rcs_.rcs_nb_bits; i++) {
            m_mb_mapping_->tab_bits[i] = 0; // 初始化为 false
    }
    ...
    // 建立传输层链接
    std::cout<<"start222"<<std::endl;
    int ret = modbus_tcp_accept(m_ctx_, &m_fd_);
    std::cout<<"start1221"<<std::endl;
    if(ret < 0) {
    ...
    std::cout<<"start22222222222"<<std::endl;
...
{
    flag_stop_thread_ = false;//线程运行停止标志位,true为停止,false为运行中
    std::cout<<"start11"<<std::endl;
    if(nullptr == thread_rcs_server_) {
        //创建新的线程对象,同时绑定成员函数ServerThread,并将当前对象的指针作为参数传递给线程函数
        thread_rcs_server_ = new std::thread(&RcsInteraction::ServerThread, this);
    }
}
//Modbus服务的主线程,维护AGV与RCS系统的双向数据流,处理通信信息
void RcsInteraction::ServerThread()
{   
    // 线程ID,用于调试和日志记录,获取到当前线程的真实ID
    ...
    status_connect_ = CONNSTATE_WAIT_FOR_CONNECT;//链接状态更新
    int rc = 0;
    // 初始化建立Socket连接
    while (!flag_stop_thread_) {  // init socket
        ...
        modbus_free(m_ctx_);//modbus内部函数,清理失败的连接上下文
        std::this_thread::sleep_for(std::chrono::milliseconds(500));//等待重联
    }
    status_connect_ = CONNSTATE_CONNECTED;//更新连接状态
    ...
    {   
        // TCP链接状态的异常处理,m_fd_为TCP链接的文件描述符
        ...
        */
        // 数据流解析
        switch (query[7]) {
            // 0x04-读输入寄存器 0x04 = “主站要我读我的输入寄存器(3x)并发数据给它”。
            ...
            // 0x10-写多个保持寄存器 0x10 = “主站要写数据到我的保持寄存器(4x)，我收下并执行它”。
            ...
            case MODBUS_FC_WRITE_MULTIPLE_COILS: {//读取
                modbus_reply(m_ctx_, query, rc, m_mb_mapping_);
                // 处理线圈状态变化
                HandleCoilOperations();
                break;
            }
            default: {
                break;
...
void RcsInteraction::IpcResDataProcess(const common_msgs::ipc_res_data& data_res)
{
    ...
    data_ipc_to_rcs_.status_charge = data_res.status_charge;
    ...
    data_ipc_to_rcs_.actual_epc_high = data_res.actual_epc_high;
    data_ipc_to_rcs_.actual_epc_low  = data_res.actual_epc_low;
    data_ipc_to_rcs_.rfid_result     = data_res.rfid_result;
    lock_.unlock();
}
...
void RcsInteraction::FillAndPubRcsTask(const uint16_t *const reg, const common_header::TaskCmdEnum& task_cmd)
{
    ...
    size_t length_info = static_cast<size_t>(sizeof(common_header::Rcs2IpcTaskInfoStru));//调度系统发布当中的“任务信息”长度
    ...
    //当调度系统传输的任务模式是4时，会对应的把具体的任务分布也发下来，数量保存在step_size中，每一个step_size占20个寄存器空间
    if(common_header::TASK_CMD_RECV == task_cmd) {
        for (size_t i = 0; i < count_one_step_task_ * size_step_task; ++i) {
            data_rcs_cmd.data_step_tasks.push_back(reg[index_step_task + i]);//从任务信息后开始读取所有的任务分布信息
        }
    } else {
...
void RcsInteraction::HandleCoilOperations()
{
    // 读取当前线圈状态
    real_time_flag_  = m_mb_mapping_->tab_bits[COIL_REAL_TIME_FLAG];
    emergency_stop_  = m_mb_mapping_->tab_bits[COIL_EMERGENCY_STOP];
    task_ready_  = m_mb_mapping_->tab_bits[COIL_TASK_READY];
    //std::cout<<"real_time_flag_: "<<real_time_flag_<<std::endl;
    //std::cout<<"emergency_stop_: "<<emergency_stop_<<std::endl;
    //std::cout<<"task_ready_: "<<task_ready_<<std::endl;
    // 检查实时标志位是否变化
    PublishCoilStatus();
}
void RcsInteraction::PublishCoilStatus()
{

    std_msgs::Int8 msg;
    msg.data = real_time_flag_;
    pub_door_stop_.publish(msg);
    
    // ROS_DEBUG("Published coil status - Real-time flag: %d, Emergency stop: %d", 
            //   real_time_flag_.load(), emergency_stop_.load());
    
}
void RcsInteraction::UpdateCoilStates()
{
    // 更新系统就绪状态
    m_mb_mapping_->tab_bits[COIL_REAL_TIME_FLAG] = system_ready_;
    
    // 更新任务就绪状态
    m_mb_mapping_->tab_bits[COIL_TASK_READY] = task_ready_;
    
    // 更新急停状态（只读，由客户端设置）
    // m_mb_mapping_->tab_bits[COIL_EMERGENCY_STOP] 由客户端写入，我们只读取
    
    // 实时通讯标志位由客户端控制，我们只读取状态
    // m_mb_mapping_->tab_bits[COIL_REAL_TIME_FLAG] 由客户端写入
}
RcsInteraction::~RcsInteraction()
{
    UninitSocket();
