...
namespace task
{
    ...
    #pragma pack(push, 1)
    struct RCSLineNodeStru  //直线行驶任务详细分布信息
    {
        RCSLineNodeStru()
            : type(1), target_vel(0), heading(0), length(0), cargo_size(0),
              start_pt_x(0), start_pt_y(0), end_pt_x(0), end_pt_y(0), map_id(0), id_a(0), id_b(0), obs_area(0)
        {
        }
    ...
    struct RCSCurveNodeStru//弯道行驶任务详细分布信息
    {
        RCSCurveNodeStru()
            : type(2), target_vel(0), heading(0), length(0), cargo_size(0),
              start_pt_x(0), start_pt_y(0), end_pt_x(0), end_pt_y(0),
              center_pt_x(0), center_pt_y(0), map_id(0), id_a(0), id_b(0), obs_area(0)
        {
        }
        void Reset(const RCSNodeStru &node);
        int16_t type;
    ...
    struct RCSWorkNodeStru//举升任务详细分布信息
    {
        RCSWorkNodeStru()
            : type(3), static_work(0), target_vel(0), stop_before_done(0),
              done_wait_time(0), cargo_size(0), is_loading_cargo(0), heading(0),
              start_pt_x(0), start_pt_y(0), position2(0), position3(0), map_id(0), id_a(0), id_b(0), obs_area(0)
        {
        }
        void Reset(const RCSNodeStru &node);
        int16_t type;
    ...
    struct RCSChargeNodeStru//充电任务详细分布信息
    {
        RCSChargeNodeStru()
            : type(4), charge_mode(0), charge_limit(0), cargo_size(0),
              heading(0), start_pt_x(0), start_pt_y(0), map_id(0), id_a(0),obs_area(0)
        {
        }
        void Reset(const RCSNodeStru &node);
        int16_t type;
    ...
    {
        RCSTransferNodeStru()
            : type(5), heading(0), start_pt_x(0), start_pt_y(0), is_where(0),
              operator_id(0), position(0), obj_speed(0), cmd(0), wait_time(0),
              cargo_size(0), map_id(0), id_a(0)
        {
        }
        void Reset(const RCSNodeStru &node);
        int16_t type;
    ...
    {
        RCSStandbyNodeStru()
            : type(8), Ope_type(0), WaitTime(0), Sen_type(0), CodeID(0), cargo_size(0),
              tar_point_x(0), tar_point_y(0), tar_point_theta(0), adj_type(0), motion_mehtod(0),
              limit_x(0), limit_y(0), map_id(0), id_a(0), id_b(0)
        {
        }
        void Reset(const RCSNodeStru &node);
        int16_t type;
    ...
    {
        RCSFinishNodeStru()
            : type(99), remain_2(0), remain_3(0), remain_4(0), remain_5(0), remain_6(0), remain_7(0), 
              remain_8(0), remain_9(0), remain_10(0), remain_11(0), remain_12(0), task_id_index(0), map_id(0),
              id_a(0), id_b(0)
        {
        }
        void Reset(const RCSNodeStru &node);
    ...
    struct RCSNodeStru//这个结构体包含了所有小任务的字段（直线、弯道、举升、充电）
    {
        RCSNodeStru()
            : type(0), target_vel(0), heading(0), length(0), cargo_size(0), static_work(0),
              stop_before_done(0), done_wait_time(0), charge_mode(0), charge_limit(0),
              operator_id(0), position1(0), obj_speed(0), cmd(0), map_id(0),
              start_pt_x(0), start_pt_y(0), end_pt_x(0), end_pt_y(0), center_pt_x(0), center_pt_y(0),
              id_a(0), id_b(0), Ope_type(0), WaitTime(0), Sen_type(0), CodeID(0), tar_point_x(0),
              tar_point_y(0), tar_point_theta(0), adj_type(0), motion_mehtod(0), limit_x(0),
              limit_y(0), obs_area(0)
        {
        }
        ...
        u_int16_t obs_area;
        //修改，新增预期EPC
        u_int16_t expected_epc_high = 0;
        u_int16_t expected_epc_low  = 0;
    };
    ...
    public:
        ...
        void CalculateLinePathDeviation(const geometry_msgs::Point32 &A, const geometry_msgs::Point32 &B, 
       ...
       private:
        ...
        void CalCurveSignRadius(const int16_t heading, const int32_t start_x, const int32_t start_y,
        ...
        void GenerateArcPoints(const geometry_msgs::Point32 &point_start, const geometry_msgs::Point32 &point_end,
                               const geometry_msgs::Point32 &center, double desired_arc_length, std::vector<geometry_msgs::Point32> &points_generate);
        void GenerateGlobalDensePath(const std::vector<RCSNodeStru> &vec_step_task_stru, std::vector<std::vector<geometry_msgs::Point32>> &points_generate);
        ...
        void GenerateAndPubFollowPathData(const u_int16_t obs_area);
        void GenerateAndPubChargeData(const int16_t charge_mode, const u_int16_t obs_area);
        void GenerateAndPubAroundSpinData(const double angle_target, const u_int16_t obs_area, const int flag_direction);
        void GenerateAndPubWorkData(const int16_t static_work, const u_int16_t obs_area);
        void GenerateAndPubPickCargoData(const u_int16_t obs_area);
        void ExecuteTask();
        void GetLookaheadPoint(geometry_msgs::Point32 &point_lookahead, const std::vector<geometry_msgs::Point32>& points_step_task);
        bool CheckNextTaskIsStopType();
        bool CheckTaskIsStopType(const int16_t type_task, const int16_t map_id);
        void GetCurrAndNextTaskInfo(
            const size_t &index_step_task, int16_t &id_curr_task, int16_t &type_curr_task, float &speed_curr_task,
            float &radius_sign_curr_task, float &radius_length_curr_task, int16_t &type_next_task,
            float &speed_next_task, float &radius_sign_next_task, int16_t& cargo_size, int16_t& map_id, u_int16_t& obs_area,int16_t& next_map_id);
        ...
        void GetHeadTailPose(const geometry_msgs::Pose &pose_current, const double yaw,
        ...
        void GetCurrentStepTaskRemainDis(DISREMAIN &dis_remain);
        bool FinishedCurrentStepTask();
        void ProcessDoorStopData(const std_msgs::Int8 &msg);
        /************* 跨楼层 配合RCS ***********/
        // bool SpecProcessForDiffFloor();
        ...
        std::vector<geometry_msgs::Point32> ConvertPath(const std::vector<geometry_msgs::PoseStamped> &path_in);
        bool GenerateGlobalPath(const geometry_msgs::PoseStamped &goal, std::vector<geometry_msgs::PoseStamped> &plan);
        
        // 计算曲率以通知PLC实现提前减速避障
        bool detectPathDeviation(const std::vector<geometry_msgs::Point32>& orig_path,
        const std::vector<geometry_msgs::Point32>& plan_path, double lateral_thresh, double heading_thresh_rad);
        void CalculateCurvePathDeviation(const std::vector<geometry_msgs::Point32>& path,
                                              const geometry_msgs::Pose& pose_current,
                                              PathDeviation& path_deviation);
        bool isOvercomeObstacle(const geometry_msgs::Pose &current_pose, const double overcome_threshold,const std::vector<geometry_msgs::Point32>& points_step_task);
        bool findNearestObstacle(const Eigen::Vector2d &P_world, std::vector<Eigen::Vector2d> &edge_points);
        bool obstacleDetection(const std::vector<geometry_msgs::Point32> &path, const geometry_msgs::Pose &current_pose,
        ...
        ros::Subscriber sub_door_stop_;

        size_t index_current_ahead_point_ = 0;
        bool flag_door_stop_ = false;
        bool flag_use_2d_loc_ = false;
        bool flag_show_project_point_ = true;
        ...
        float mm_to_m = 0.001;
        float m_to_mm = 1000.0;
        double threshold_arrive_des_ = 1.0;//自动吸附的允许误差
        double charge_arrive_des_ = 0.03;
        u_int8_t id_marker_ = 0;
        geometry_msgs::Pose pose_current_;
        geometry_msgs::Twist twist_current_;
        double yaw_current_ = 0;
        double vel_current_ = 0;
        ...
        int16_t cargo_size_ = 0;
        int16_t map_id_ = 0;
        int16_t next_map_id = 0;
        std::vector<geometry_msgs::PoseStamped> path_global_;

        // ipc res task info
        size_t index_current_step_task_ = 0;    //当前子任务标号
        int16_t compelete_percent_ = 0;     //完成百分比
        ...
        int8_t  g_id_floor_ = 0;      // 当前车子所处的楼层号码
        ...
        u_int16_t obs_area_ = 0;

        // action data to plc
        int32_t id_action_current_ = 1; // 下发动作序列号，从1开始，如果从0开始，PLC默认回复0会导致刚好对上
        
        
        int flag_direction_ = 0;
        bool spin_initialized_ = false;     // 是否已初始化自旋起点和目标
        double spin_start_yaw_ = 0.0;       // 自旋起始yaw
        double spin_total_angle_ = 0.0;     // 总旋转角度（方向已确定）
        bool has_active_obstacle = false;
        double obs_proj_path = 0.0;

        
        std::vector<std::string> rfid_last_scan_result_;  // 保存上一次扫描结果，供后续比较
        RFIDReader rfid_;
        bool flag_rfid_enabled_      = false;
        bool flag_rfid_scan_started_ = false;   // 本轮叉取是否已触发扫描
