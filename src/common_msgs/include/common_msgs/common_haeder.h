...
enum IPC_ERROR_CODE
{
    ...
    ERROR_RCS_PATH_CHECK_FAILED,       //RCS: RCS下发点不连续
    ...
    ERROR_PALLET_CAMEAR_INIT_FAILED,      //算法(相机): 相机初始化失败
    // error_2
...
typedef struct {
    ...
    int16_t status_charge = 0;                   // 充电状态，0-未知；1-充电中；2-充电结束
    ...
    //修改，添加字段
    uint16_t actual_epc_high = 0;    // 实际读到的EPC高2字节（前4个十六进制字符）
    uint16_t actual_epc_low  = 0;    // 实际读到的EPC低2字节（后4个十六进制字符）
    uint16_t rfid_result     = 0;    // 0=未执行，1=匹配，2=不匹配，3=未读到标签
} Ipc2RcsDataStru;
